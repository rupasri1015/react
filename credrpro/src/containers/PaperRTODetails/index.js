...
class PaperRTODetails extends Component {
  state = {
    currentTab: 0,
    imageurls: [],
    currentIndex: 0,
  ...
  onChangeVahaanLink = (e) => {
    this.setState({
      vaahanLink: e.target.value
    })
  }
  ...
  updatePaperDocPayload = () => {
    const { approveStatus, imageurls, Data, vehicleDetail, imageURLS, vaahanLink } = this.state
    const { dispatch } = this.props
    const payload = { leadId: this.props.match.params.leadId, ptStatus: 'VAAHAN_APPROVED', customerMobileNumber: Data.customerMobileNumber, ptCurrentStage: 3, vaahanLink: vaahanLink, customerName: Data.customerName, cityName: Data.cityName }
    if(!payload.vaahanLink || payload.vaahanLink === '') {
      dispatch(setNotification('danger', 'Error', 'Please Enter Vahaan link'))
    }
    else {
  ...
  render() {
    const { currentTab, imageOpen, images, currentIndex, qcBikeDetails, vehicleDetail, imageUrl, vaahanLink } = this.state
    return (
      <Fragment>
        {
          !qcBikeDetails.length ? <NoResultsFound /> :
            < div className="tab-container" >
              ...
              >
                ...
                <Tab label="Vaahan Link" className="tab-style" {...this.tabProps(4)} />
              </Tabs>
              {
              ...
              }
              {
                <TabPanel value={currentTab} index={qcBikeDetails.length + 1}>
                  <h1 className="tab-heading">Vahaan Link</h1>
                  <div className="row">
                    <div className="col-12 doc-received-header">Upload Vahaan Link*</div>
                    <div className="col-6 upload-image">
                      <Input value={vaahanLink} onChange={(e) => this.onChangeVahaanLink(e)}></Input>
                    </div>
                  </div>
                </TabPanel>
              }
...
export default connect()(PaperRTODetails)
