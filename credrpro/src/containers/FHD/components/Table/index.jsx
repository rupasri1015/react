...
import { getFhdList, resetFhdList } from '../../../../redux/actions/fhdListAction'
...
class FhdTable extends Component {
  ...
  componentWillUnmount() {
    const { dispatch } = this.props
    dispatch(resetFhdList())
  }

  render() {
    const { fhdList, total, page, onUpdateFHD } = this.props
    return (
      <div className="table-wraper marginTop">
        <Table size="small">
          ...
          <TableBody>
            {
              Boolean(fhdList.length) && fhdList.map(fhd => {
                const {
                  storeName,
                  userFirstName,
                  userMobileNumber,
                ...
                return (
                  <TableRow hover tabIndex={-1} key={userId}>
                    ...
                    <TableCell>
                      <img src={edit} id={userId} alt="Edit Valuator" role="button" className="action-icon" onClick={() => onUpdateFHD(fhd)} />
                    </TableCell>
                  </TableRow>
                )
              })
...
const mapStateToProps = (state) => ({
  fhdList: state.fhdData.fhdList,
  total: state.fhdData.total,
  page: state.fhdData.page
})

export default connect(mapStateToProps)(FhdTable)