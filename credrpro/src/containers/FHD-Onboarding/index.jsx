...
import { FHD_SHD_STEPS, FHD_USER_TYPES } from '../../core/constants/settings'
...
import { addFHD } from '../../core/services/fhdServices'
...
class AddFHD extends Component {
  ...
  handleBankForm = bankInfo => {
    ...
    const payload = { oemInfo, mailingAddress, registeredAddress, loginDetails, bankInfo, storeType: 'FHD' }
    addFHD(payload)
      .then(apiResponse => {
        if (apiResponse.isValid) {
          dispatch(setNotification('success', 'Success', apiResponse.message))
          destroy('outlet-details')
          destroy('profile-details')
          destroy('bank-details')
          history.push('/fhd')
