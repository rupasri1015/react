...
const DocumentAndImages = ({ userDocs, vehicleDocs, kycDocs, deliveryNotes, checkData }) => {
    ...
    <Fragment>
      ...
      <div className="doc-container">
        ...
        <div className="docs">
          ...
          {
            checkData === "new" ?
              vehicleDocs && vehicleDocs.length && vehicleDocs.map((img, index) => {
                if (img.leadParameterType === 'IMAGE') {
                  return (
                    <div className="doc-preview" key={img.leadParameterValue} onClick={() => openImageViewer(vehicleDocs, index, 'leadParameterValue', 'leadParameterName')}>
                      <img src={img.leadParameterValue} alt={img.leadParameterName} />
                    </div>
                  )
                }
              })
        ...
        <div className="docs">
          {
            ...
            checkData === "new" ?
              vehicleDocs && vehicleDocs.length && vehicleDocs.map((img, index) => {
                if (img.leadParameterType === 'VIDEO') {
                  return (
                    <div className="doc-preview" key={img.leadParameterValue} onClick={() => openImageViewer(vehicleDocs, index, 'leadParameterValue', 'leadParameterName')}>
                      <video
                        src={img.leadParameterValue}
                        key={img.leadParameterValue}
                        height="250"
                        width="100%"
                        controls
                      >
