...
import VehicleDetails from './component/vehicleDetail'
...
class PaperDetailsView extends Component {
  state = {
    currentTab: 0,
    imageurls: [],
    currentIndex: 0,
  ...
  updatePaperDocPayload = () => {
    ...
    const { dispatch } = this.props
    const payload = { leadId: this.props.match.params.leadId, ptStatus: 'RTO_SUBMITTED', customerMobileNumber: Data.customerMobileNumber, ptCurrentStage: 2, imageUrl: imageURLS, customerName: Data.customerName, cityName: Data.cityName }
    if(!payload.imageUrl.length) {
      dispatch(setNotification('danger', 'Error', 'Please Upload RC Receipt'))
    }
    else {
          ...
          !qcBikeDetails.length ? <NoResultsFound /> :
            < div className="tab-container" >
              ...
              >
                ...
                <Tab label="RTO Recipt" className="tab-style" {...this.tabProps(4)} />
              </Tabs>
              {
              ...
              }
              {
                <TabPanel value={currentTab} index={qcBikeDetails.length + 1}>
                  <h1 className="tab-heading">RTO Receipt</h1>
                  <div className="row">
...
export default connect()(PaperDetailsView)
